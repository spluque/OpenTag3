...
## Surface pressure estimate in mbar for depth calculation depth
## calculation assumes 1 bar = 10 m
surfacepress = 1010;

## ans = input('Append? 0=No  1=Yes ');
ans = 0;
[FileName, PathName, FilterIndex] = uigetfile({'*.DSG', 'DSG files (*.DSG)'},
					      'Select a DSG file');
...
for n=1:length(SID_SPEC)	# Loop through all SIDs and reassign
  ...
  if (SID_SPEC(n).SID == 'INER')
	...
	## calibrate accelerometer
        INER.accel = [INER.accel accel_cal.*(INER.data(k:INER.nchan:end))];
      end
    end
    m = k;
	...
	## calibrate magnetometer
        INER.mag = [INER.mag mag_cal.*(INER.data(m:INER.nchan:end))];
      end
    end
    if(bitand(INER.SensorType,8))
      for j=m+1:m+3		# gyros
	## calibrate gyro
        INER.gyro = [INER.gyro gyro_cal.*(INER.data(j:INER.nchan:end))];
      end
    end
  end
...
if (isempty(PTMP) == 0)
  ...
  if (fid < 1)
    ...
    msg = 'Select presstemp.cal Calibration File'
    [FileName, PathName, FilterIndex] = uigetfile({'*.cal', 'cal files (*.cal)'},
						  msg);
    if isequal(FileName, 0) | isequal(PathName, 0)
      return
    end
  ...
  for n=1:6:length(PTMP.data)
    p2bits = bitor(bitshift(uint64(PTMP.data(n)), 16),
		   bitshift(uint64(PTMP.data(n + 1)), 8));
    ## Pressure measurement
    D1 = double(bitor(p2bits, uint64(PTMP.data(n + 2))));

    t2bits = bitor(bitshift(uint64(PTMP.data(n + 3)), 16),
		   bitshift(uint64(PTMP.data(n + 4)), 8));
    ## Temperature measurement
    D2 = double(bitor(t2bits, uint64(PTMP.data(n + 5))));
    ...
    pressure(counter) = (D1 * SENS / 2097152 - OFF) / 81920;
    counter = counter + 1;
  end
