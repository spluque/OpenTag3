function [DF_HEAD, SID_SPEC, SID_REC,pos] = oDSG(FileName)
  ...
  while (notdone)
    ...
    ## Sample period (us) x 256
    SID_SPEC(nSIDSPEC).SPus = fread(fid, 1, 'uint32');
  ...
  while (eofstat == 0)
    ...
    ##  Number of bytes recorded since start of sampling for this SID
    SID_REC(reccounter).nbytes = fread(fid, 1, 'uint32');
    if (DF_HEAD.Version > 9999)
        ## roll counter
        SID_REC(reccounter).nbytes_2 = fread(fid, 1, 'uint32');
    end
	...
	## 24-bit samples read in 8 bits at a time
        SID_REC(reccounter).data = fread(fid, nsamples, 'uint8');
      end
    else
